...
float pressureThreshold = 0;//åŽ‹åŠ›æœ‰æ•ˆçš„ä¸‹é™

int channelchange = 0;
uint8_t tempRxBuffer;
uint8_t upperRxBuffer[7];
int cnt = 0;
volatile int testmode_flag = 6;//Í¨¹ýÐÞ¸Ä´Ë´¦¿ÉÒÔÐÞ¸ÄÆô¶¯µÄÄ£Ê½
			...
			if(sum==UART8RxBuff){
				...
				switch (upperRxBuffer[0]){
					case 8:{
						...
						pressureThreshold = upperRxBuffer[6]/10.0;//åŽ‹åŠ›æ¿€æ´»çš„ä¸‹é™
						return;
					}
					case 9:{
						//ç”µæµå¹…å€¼ä¸Šé™
				...
				//frequency ´ÓPCÖÐÏÈÊÕµ½µÍÎ»ÔÙÊÕµ½¸ßÎ»
				...
				//times´Ì¼¤´ÎÊý£¬ºÍfrequency½áºÏÆðÀ´Ê¹ÓÃ
...
/*ÖØ¶¨Ïòc¿âº¯Êýprintfµ½DEBUG_USARTX*/
