...
void merge_stimulate_parameter(UART_HandleTypeDef *huart,float pressure,int channel)
{			
	if(pressure<0)
		pressure = 0;
	...
	if(testmode_flag==1||testmode_flag==4){
		...
		stimulate_parameter[7] = (25+(int)pressure/2)>>8; //é¢‘ç‡é«˜8ä½
		stimulate_parameter[8] = 25+(int)pressure/2; //é¢‘ç‡ä½8ä½
	}		
		...
		stimulate_parameter[16]=threshold[channel][0];//é…åˆè„‰å®½æ¨¡å¼å’Œè¯±å‘æŒ‡æ„ŸåŒºçš„æœ€ä½ç”µæµé˜ˆå€¼ï¼Œåœ¨è„‰å®½æ¨¡å¼ä¸‹ä¹Ÿä¼š
	}
	else if(testmode_flag==7)
	{
		...
		if(pressure<pressureThreshold||channelEnableflag[channel]==0){
			if(testmode_flag!=7)
